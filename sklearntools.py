...
#         self_class_name = name_estimator(self)
...
#         self_name = self._get_name([])
#         return MultipleResponseEstimator([(self_name, mask, self)])
...
#     def _pad_right(self, n):
#         new_estimators = []
#         for name, mask, estimator in self.estimators:
#             new_mask = np.concatenate([mask, np.zeros(n, dtype=bool)], axis=1)
#             new_estimators.append((name, new_mask, estimator))
#         new_args = self.get_params()
#         new_args['estimators'] = new_estimators
#         return self.__class__(**new_args)
#     
#     def _pad_left(self, n):
#         new_estimators = []
#         for name, mask, estimator in self.estimators:
#             new_mask = np.concatenate([np.zeros(n, dtype=bool), mask], axis=1)
#             new_estimators.append((name, new_mask, estimator))
#         new_args = self.get_params()
#         new_args['estimators'] = new_estimators
#         return self.__class__(**new_args)
# #     
#     def __and__(self, other):
#         if not is_masked(other):
#             other = mask_estimator(other, slice(None))
#         if not compatible_masks(self._masks() + other._masks()):
#             raise ValueError('Masks do not have compatible sizes')
