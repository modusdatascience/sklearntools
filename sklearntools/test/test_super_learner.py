...
def test_super_learner():
    ...
    model = CrossValidatingEstimator(SuperLearner([LinearRegression(), Earth(max_degree=2)],
                         LinearRegression(), cv=5, n_jobs=1), cv=5)
    cv_pred = model.fit_predict(X, y)
    pred = model.predict(X)
    cv_r2 = r2_score(y, cv_pred)
    best_component_cv_r2 = max([r2_score(y, model.estimator_.cross_validating_estimators_[i].cv_predictions_) for i in range(2)])
    ...
    print(max([r2_score(y, model.estimator_.cross_validating_estimators_[i].cv_predictions_) for i in range(2)]))
